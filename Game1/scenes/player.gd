...
@onready var dash_decel_time = $DashDecelTime
...
func _physics_process(delta):
	...
	if !dash_buffer_timer.is_stopped() && dash_cooldown.is_stopped() && direction && CANMOVE  && !isAttacking && is_on_floor() && GETMOVEINPUT && !isParrying:
		dash()
	if !CANMOVE && !dash_decel_time.is_stopped():
		dashdecel = move_toward(dashdecel, 1000, 50)
		velocity.x = move_toward(velocity.x, 0, dashdecel)
	
	#Dashing animation
	if isDashing:
		sprite_2d.scale.x = 0.655
		sprite_2d.scale.y = 0.266
		sprite_2d.rotation = 0
	
	# Animation running
	if velocity.x != 0 && !isDashing && is_on_floor() && !isAttacking && GETMOVEINPUT && !isParrying:
		running_particles.emitting = true
		if !SWORD:
			if sprite_2d.flip_h:
				sprite_2d.animation = "runningL"
			else:
				sprite_2d.animation = "running"
		if SWORD:
			if BLACKMODE:
				if sprite_2d.flip_h:
					sprite_2d.animation = "runningswordLB"
				else:
					sprite_2d.animation = "runningswordB"
			else:
				if sprite_2d.flip_h:
					sprite_2d.animation = "runningswordL"
				else:
	...
	# Animation idle
		...
		if !isDashing && !isAttacking && !isParrying:
			if !SWORD:
				if sprite_2d.flip_h:
					sprite_2d.animation = "idleL"
				else:
					sprite_2d.animation = "idle"
			if SWORD:
				if BLACKMODE:
					if sprite_2d.flip_h:
						sprite_2d.animation = "idleswordLB"
					else:
						sprite_2d.animation = "idleswordB"
				else:
					if sprite_2d.flip_h:
						sprite_2d.animation = "idleswordL"
					else:
						sprite_2d.animation = "idlesword"
			sprite_2d.scale.x = 0.432
			sprite_2d.scale.y = 0.432
			sprite_2d.rotation = 0
	...
	#move
	if direction && CANMOVE && GETMOVEINPUT && !animation_player.current_animation == "attack1":
		velocity.x = move_toward(velocity.x, direction * SPEED, ACCEL)
	elif !animation_player.current_animation == "attack1":
	...
	if !attack_buffer_timer.is_stopped() && SWORD && CANMOVE && GETMOVEINPUT && attack_cooldown.is_stopped() && !isAttacking && !isParrying:
		if !sprite_2d.flip_h:
	...
	if attack_duration.is_stopped() && isAttacking:
		isAttacking = false
	...
	if !parry_buffer_timer.is_stopped() && SWORD && CANMOVE && GETMOVEINPUT && parry_cooldown.is_stopped() && !isAttacking && !isParrying:
		#parry()
...
func _on_attack_hitbox_area_entered(area):
	if area.is_in_group("enemiehitbox"):
		area.get_parent().take_damage(global_position, 34)
...
func _on_dash_duration_timeout():
	dash_decel_time.start()

func _on_dash_decel_time_timeout():
	CANMOVE = true
	isDashing = false
	dashdecel = 0
